...
def load_model():
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"Model.pkl was not found at: {model_path}"
        )
...
if list(input_df.columns) != EXPECTED_FEATURES:
    ...
    st.error("❌ Model feature order is incorrect.")
...
if predict_btn:
    ...
    try:
        ...
        try:
            ...
            # Get classifier classes from a Pipeline or directly from the model.
            final_model = None

            if hasattr(model, "named_steps"):
                final_model = model.named_steps.get("model", None)
                if final_model is None and len(model.named_steps) > 0:
